...
export function getTopLevelReadableElementsOnPage(): HTMLElement[] {
  let rootElement = document.querySelector("#content-1");
  let readableSection = rootElement?.querySelector("section");
  let toplevelElems: HTMLElement[] = [];
  if (readableSection?.children) {
    let readableElements = readableSection.querySelectorAll("*");
    readableElements.forEach((elem: any) => {
      let tagName = elem.nodeName;
      if (!IGNORE_LIST.includes(tagName) && elem.textContent?.length) {
        toplevelElems.push(elem);
      }
    });
  }
